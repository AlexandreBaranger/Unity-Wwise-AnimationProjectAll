...
using System.IO;
using UnityEditor;
public class SynthWorkingPresetCSV : MonoBehaviour
{
    ...
    [Range(0f, 100f)]
    public float LFODepthRandomRange = 2f;
    [Range(0f, 20000f)]
    public float LFOFrequencyRandomRange = 2f;
    ...
    {
        get => preset1Selected;
        set {preset1Selected = value;if (value){ApplyPreset(preset1);Preset2 = false;Preset3 = false;}}
    }

    public bool Preset2
    {
        get => preset2Selected;
        set {preset2Selected = value;if (value){ApplyPreset(preset2);Preset1 = false;Preset3 = false;}}
    }

    public bool Preset3
    {
        get => preset3Selected;
        set{preset3Selected = value;if (value){ApplyPreset(preset3);Preset1 = false;Preset2 = false;}}
    }
    ...
    private void ApplyPreset(Preset preset)
    {
        ...
        float lfoDepth = preset.sliderRtpcLFODepth + Random.Range(-LFODepthRandomRange, LFODepthRandomRange);
        float lfoFrequency = preset.sliderRtpcLFOFrequency + Random.Range(-LFOFrequencyRandomRange, LFOFrequencyRandomRange);
        ...
        transpose = Mathf.Clamp(transpose, -3600f, 3600f);
        ...
        lfoDepth = Mathf.Clamp(lfoDepth, 0f, 100f);
        lfoFrequency = Mathf.Clamp(lfoFrequency, 0f, 20000f);
        // Debug.Log($"Applying preset: Volume = {volume}, LFODepth = {lfoDepth}, LFOFrequency = {lfoFrequency},Synth3P = {pitch}, Synth3T = {transpose}, Synth3S = {speedPlay}, Synth4 = {attackCurve}, Synth5 = {attackTime}, Synth6 = {decayTime}");
        ...
        rtpcLFODepth.SetGlobalValue(lfoDepth);
        rtpcLFOFrequency.SetGlobalValue(lfoFrequency);
    }
    public void ApplyPresetByType(PresetType presetType)
    {
        switch (presetType)
        {
            ...
            case PresetType.Preset3:
                Preset3 = true;
                break;
            default:Debug.LogWarning($"Preset '{presetType}' not found.");
                break;
        }
    }
    void Start()
...
#if UNITY_EDITOR
    [CustomEditor(typeof(SynthWorkingPresetCSV))]
    public class SynthWorkingPresetCSVEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
            SynthWorkingPresetCSV script = (SynthWorkingPresetCSV)target;
        ...
        private void SavePresetToCSV(SynthWorkingPresetCSV script)
        {
